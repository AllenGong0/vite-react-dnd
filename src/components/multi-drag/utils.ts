...
const reorderMultiDrag = ({ entities, selectedTaskIds, source, destination }: Args): Result => {
  ...
  const insertAtIndex: number = (() => {
    const destinationIndexOffset: number = selectedTaskIds.reduce((previous: number, current: TaskId): number => {
      if (current === dragged) {
        return previous;
      }
  ...
  console.log('insert at index:', insertAtIndex);
  ...
  console.log('ordered selected tasks:', orderedSelectedTaskIds);

  // we need to remove all of the selected tasks from their columns
  const withRemovedTasks: ColumnMap = entities.columnOrder.reduce((previous: ColumnMap, columnId: Id): ColumnMap => {
    const column: Column = entities.columns[columnId];

    // remove the id's of the items that are selected
    const remainingTaskIds: TaskId[] = column.taskIds.filter((id: TaskId): boolean => !selectedTaskIds.includes(id));

    previous[column.id] = withNewTaskIds(column, remainingTaskIds);
    return previous;
  }, entities.columns);

  console.log('with removed tasks:', withRemovedTasks);
  ...
  console.log('with inserted tasks:', withInserted);
  ...
  console.log('final updated:', updated);
